...
function CreateProduct1(product_name, brand, reviews, price, rating) {
  ...
  Object.setPrototypeOf(obj, CreateProduct1.prototype);
...
function CreateProduct2(product_name, brand, reviews, price, rating) {
  ...
  this.price = price;
  this.rating = rating;
  //  Object.setPrototypeOf(this, CreateProduct2.prototype);
}
...
function removeKeyValuePair(user, keyToRemove) {
  ...
  delete updatedUser[keyToRemove];
  return updatedUser;
}
...
function massageArray(inputArray) {
  const resultArray = inputArray.map((item) => {
    ...
    const area = areas.find((area) => area.id === item.Area).name;
    const ingredients = [];
    for (let i = 1; i <= 20; i++) {
